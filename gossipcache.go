...
func NewGossipCacheCluster(conf *Config, options ...ClusterConfigOption) (*GossipCache, error) {
	...
	if logger := conf.Logger; logger != nil {
		SetGroupCacheLogger(logger) // unfortunatelly it's global
	...
	gc.pool = groupcache.NewHTTPPoolOpts(conf.CacheBaseURL, &groupcache.HTTPPoolOptions{
